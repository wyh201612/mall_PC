<template>
    <div class="main-content">
        <div class="title margin-bottom-sm">
            <el-divider direction="vertical" class="bg-orange"></el-divider>我的收货地址
        </div>
        <el-card class="box-card" shadow="hover" style="margin-bottom: 20px;" v-for='(item, index) in addressList' :key="index">
            <div :class="item.isDefault==1?'address-box':''" style="position: relative;font-size: 14px;">
                <span class="el-icon-close" style="position: absolute;top: 0;right: 0;cursor: pointer;" @click='del_address(item.id, item.isDefault)'></span>
                ...
                <div>
                    <span class="el-icon-location" :style="item.isDefault==1?'color: var(--orangeLight)':''"></span>
        ...
        <el-button class="addAddress" icon="el-icon-plus" @click='add_addressShow("ruleForm")'>添加地址</el-button>
...
<style lang="scss" scoped>
    ...
    .address-box::before {
        ...
        border-left: 50px solid var(--orangeLight);
        position: absolute;
        bottom: -20px;
        left: -20px;
    }

    .addAddress.el-button {
        color: var(--white);
